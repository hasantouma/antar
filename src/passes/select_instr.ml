...
let var_of_stmt (s : Clang.stmt) : Xlang.Ast.var =
  match s with
  | Set (x, _) -> x

let rec get_vars (acc : Clang.var list) (tail : Clang.tail) : Xlang.Ast.var list =
  match tail with
  | Return _ -> acc
  | Seq (s, t) ->
...
let select_arg (arg : Clang.arg) : Xlang.Ast.arg =
  match arg with
  | Number n -> Constant n
  | Var s -> Ref s

let select_exp (dst : Xlang.Ast.arg) (exp : Clang.exp) : Xlang.Ast.instr list =
  match exp with
  | Arg arg ->
...
let select_stmt (stmt : Clang.stmt) : Xlang.Ast.instr list =
  match stmt with
  | Set (var, exp) ->
    let arg_val = select_arg (Var var) in
    select_exp arg_val exp

let rec select_tail (tail : Clang.tail) : Xlang.Ast.instr list =
  match tail with
  | Return arg ->
    let arg' = select_arg arg in
    [ Movq (arg', Reg RAX) ]
...
let select_instr (p : Clang.cprogram) : Xlang.Ast.xprogram =

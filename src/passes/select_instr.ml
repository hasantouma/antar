...
let var_of_stmt (s : Clang.Ast.stmt) : X0.Ast.var =
  match s with
  | Set (x, _) -> x

let rec get_vars (acc : Clang.Ast.var list) (tail : Clang.Ast.tail) : X0.Ast.var list =
  match tail with
  | Return _ -> acc
  | Seq (s, t) ->
...
let select_arg (arg : Clang.Ast.arg) : X0.Ast.arg =
  match arg with
  | Number n -> Constant n
  | Var s -> Ref s

let select_exp (dst : X0.Ast.arg) (exp : Clang.Ast.exp) : X0.Ast.instr list =
  match exp with
  | Arg arg ->
...
let select_stmt (stmt : Clang.Ast.stmt) : X0.Ast.instr list =
  match stmt with
  | Set (var, exp) ->
    let arg_val = select_arg (Var var) in
    select_exp arg_val exp

let rec select_tail (tail : Clang.Ast.tail) : X0.Ast.instr list =
  match tail with
  | Return arg ->
    let arg' = select_arg arg in
    [ Movq (arg', Reg RAX) ]
...
let select_instr (p : Clang.Ast.cprogram) : X0.Ast.xprogram =

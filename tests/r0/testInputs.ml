...
(* int *)
let one = { ast = `EInt 1; optimized = `EInt 1; interp = 1; input = []; message = "int (1)" }

let forty_two = { ast = `EInt 42; optimized = `EInt 42; interp = 42; input = []; message = "int (2)" }

let int_list = [ one; forty_two ]

(* read *)
let read = { ast = `ERead; optimized = `ERead; interp = 23; input = [ 23 ]; message = "read (1)" }

let read_list = [ read ]

(* add *)
let add_1_2 = { ast = `EAdd (`EInt 1, `EInt 2); optimized = `EInt 3; interp = 3; input = []; message = "add (1)" }
...
(* negate *)
let negate_1 = { ast = `ENegate (`EInt 1); optimized = `EInt (-1); interp = -1; input = []; message = "negate (1)" }

let negate_read =
  { ast = `ENegate `ERead; optimized = `ENegate `ERead; interp = -12; input = [ 12 ]; message = "negate (2)" }
...
let negate_negate_5 =
  { ast = `ENegate (`ENegate (`EInt 5)); optimized = `EInt 5; interp = 5; input = []; message = "negate (5)" }

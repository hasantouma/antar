...
let compiler (expr : Rlang.expr) : Xlang.Ast.xprogram =
  expr |> Rlang.optimize |> Passes.Uniquify.uniquify |> Passes.Resolve_complex.resolve_complex
...
let test_interp (t : rtest) =
  ...
  assert_equal t.value (Rlang.interp ~inputs:t.inputs p.e) ~msg:("interp: " ^ t.message) ~printer:string_of_int;
...
let test_optimize (t : rtest) =
  ...
  assert_equal
    (Rlang.interp ~inputs:t.inputs p_optimized.e)
    (Rlang.interp ~inputs:t.inputs p_expr.e)
    ~msg:("optimize vs. non-optimize: " ^ t.message)
    ~printer:string_of_int;
  assert_equal p_optimized.e (Rlang.optimize p_expr.e)
